...
namespace NBearLite.DbProviders
{
    ...
    public sealed class DbProviderFactory
    {
        ...
        public static DbProvider CreateDbProvider(string assemblyName, string className, string connStr)
        {
            ...
            if (connStr.ToLower().Contains("microsoft.jet.oledb") || connStr.ToLower().Contains(".db3") || className.ToLower() == "System.Data.SqlServerCe".ToLower())
            {
                ...
                else if (mdbPath.StartsWith("../"))
                {
                    mdbPath = Path.GetFullPath(AppDomain.CurrentDomain.BaseDirectory.TrimEnd('\\') + "\\" + mdbPath);
                    //mdbPath = mdbPath.Replace("/", "\\").Replace(".\\", AppDomain.CurrentDomain.BaseDirectory.TrimEnd('\\') + "\\");
                }

                connStr = connStr.Substring(0, connStr.ToLower().IndexOf("data source")) + "Data Source=" + AppDomain.CurrentDomain.BaseDirectory + mdbPath;
            }
            ...
            else if (className.ToLower() == "System.Data.SqlServerCe".ToLower())
            {
                assemblyName = "NBearLite.AdditionalDbProviders";
                className = "NBearLite.DbProviders.SqlServerCE.SqlServerCEDbProvider";
            }
