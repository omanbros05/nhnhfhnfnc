...
{
    [Synchro]
    public partial class CompanyDangerChemicals
    {
        ...
        internal EList<CompanyDangerChemicals> CompanyDangerChemicalsGetAll(int page, int pageSize, string name, int IsCity, int AreaID)
        {
            EList<CompanyDangerChemicals> Companys = new EList<CompanyDangerChemicals>();
            ...
            if (Clips.Count > 0)
            {
                section.Where(DataBases.CompanyDangerChemicals.CompanyID.In(DB.Select(DataBases.Company, DataBases.Company.CompanyID).Where(Clips).ToSubQuery()));
            }
            ...
            if (count > 0)
            {
                ...
                using (IDataReader reader = section.ToDataReader())
                {
                    while (reader.Read())
                    {
                        CompanyDangerChemicals areaCode = ReadRow(reader);
                        Companys.Items.Add(areaCode);
                    }
                }
                int k = 0;
                foreach (CompanyDangerChemicals company in Companys.Items)
                {
        ...
        protected override void BeginUpdate(System.Data.Common.DbTransaction trans)
        {
            ...
            Company.CompanyType = (int)Company.ECompanyType.危险化学品经营许可表;
