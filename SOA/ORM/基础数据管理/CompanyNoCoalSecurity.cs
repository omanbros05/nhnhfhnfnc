...
namespace SOA.ORM
{
    public partial class CompanyNoCoalSecurity
    {
        ...
        internal EList<CompanyNoCoalSecurity> CompanyNoCoalSecurityGetAll(int page, int pageSize, string name, int IsCity, int AreaID)
        {
            EList<CompanyNoCoalSecurity> Companys = new EList<CompanyNoCoalSecurity>();
            SelectSqlSection section = DB.Select(DataBases.CompanyNoCoalSecurity, DataBases.CompanyNoCoalSecurity.ID.Count());
            ...
            if (Clips.Count > 0)
            {
                section.Where(DataBases.CompanyNoCoalSecurity.CompanyID.In(DB.Select(DataBases.Company, DataBases.Company.CompanyID).Where(Clips).ToSubQuery()));
            }
            ...
            if (count > 0)
            {
                section = DB.Select(DataBases.CompanyNoCoalSecurity);
                if (Clips.Count > 0)
                {
                    section.Where(DataBases.CompanyNoCoalSecurity.CompanyID.In(DB.Select(DataBases.Company, DataBases.Company.CompanyID).Where(Clips).ToSubQuery()));
                }

                section.SetSelectRange(pageSize, (pageSize * (page - 1)), DataBases.CompanyNoCoalSecurity.ID);

                using (IDataReader reader = section.ToDataReader())
                {
                    while (reader.Read())
                    {
                        CompanyNoCoalSecurity areaCode = ReadRow(reader);
                        Companys.Items.Add(areaCode);
                    }
                }
                int k = 0;
                foreach (CompanyNoCoalSecurity company in Companys.Items)
                {
        ...
        /// <returns>实体</returns>
        internal CompanyNoCoalSecurity CompanyNoCoalSecurityGetOne(int CompanyID)
        {
            CompanyNoCoalSecurity Company = new CompanyNoCoalSecurity();
            SelectSqlSection section = DB.Select(DataBases.CompanyNoCoalSecurity, DataBases.CompanyNoCoalSecurity.ID.Count());
            if (CompanyID >= 0)
            {
                section.Where(DataBases.CompanyNoCoalSecurity.CompanyID == CompanyID);
            }
        ...
        protected override void BeginUpdate(System.Data.Common.DbTransaction trans)
        {
            ...
            Company.CompanyType = (int)Company.ECompanyType.非煤矿山安全生产许可企业详情表;
